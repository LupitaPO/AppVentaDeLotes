...
// ATAMAINE: URL base del backend .NET donde consultamos la lista real de asesores.
...
// ATAMAINE: Normalizamos los datos del asesor para que el reporte siempre tenga las mismas columnas visibles.
...
const ReporteAsesores = ({ navigation }: ReporteAsesorProps) => {
	...
	// ATAMAINE: Al entrar a la pantalla cargamos una sola vez todos los asesores desde el API en TIEMPO REAL.
	useEffect(() => {
		const cargarAsesorIniciales = async () => {
			try {
				setCargando(true);
				setMensaje("");
				console.log("📡 Llamando API: " + API_URL + "/Asesor/reporte_Asesores/*");
				const response = await fetch(`${API_URL}/Asesor/reporte_Asesores/${"*"}`);
				...
				console.log("✅ Respuesta cruda del API (Asesores):", rawData);
				const asesorBase = normalizarAsesores(parseReporteResponse(rawData));
				console.log("✅ Asesores normalizados cargados:", asesorBase.length, asesorBase);
				setTodosAsesores(asesorBase);
				setReporte(asesorBase);
				setBuscado(false);
			} catch (error) {
				console.error("❌ Error al cargar asesores base:", error);
				setTodosAsesores([]);
				setReporte([]);
	...
	const consultarReporte = async () => {
		...
		try {
			...
			const response = await fetch(`${API_URL}/Asesor/reporte_Asesores/${filtro}`);
